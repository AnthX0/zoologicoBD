...
public class ItinerarioDAO implements IitinerarioDAO {
    ...
    public Itinerario agregar(Itinerario itinerario) {
        ...
        documentoItinerario.append("duracionRecorrido", 
                itinerario.getDuracionRecorrido());
        documentoItinerario.append("longitud", 
                itinerario.getLongitud());
        documentoItinerario.append("maxNumVisitantes", 
                itinerario.getMaxNumVisitantes());
        GregorianCalendar fechaHoraItinerario = new GregorianCalendar(
                itinerario.getFechaHoraItinerario().getYear(),
                // El mes en GregorianCalendar es base 0
                itinerario.getFechaHoraItinerario().getMonth() - 1,
        ...
        documentoItinerario.append("fechaHoraItinerario", new Date(
                timestamp));
    ...
    public List<Itinerario> consultarItinerarios() {
        ...
        try (MongoCursor<Document> cursor = documentos.iterator()) {
            while (cursor.hasNext()) {
                ...
                // Crear una instancia de Itinerario y asignar los
                // valores del documento
                Itinerario itinerario = new Itinerario();
                itinerario.setId(documento.getObjectId("_id"));
                itinerario.setNombre(documento.getString("nombre"));
                itinerario.setDuracionRecorrido(
                        documento.getInteger(
                                "duracionRecorrido"));
                itinerario.setLongitud(documento.getDouble(
                        "longitud").floatValue());
                itinerario.setMaxNumVisitantes(
                        documento.getInteger(
                                "maxNumVisitantes"));
                itinerario.setFechaHoraItinerario(
                        documento.getDate(
                                "fechaHoraItinerario"));
                List<Document> zonasDoc = (List<Document>) documento.get(
                        "zonas");
                List<Zona> zonas = new ArrayList<>();
                for (Document DocumentoZona : zonasDoc) {
                 
                    Zona zona = new Zona();
                    zona.setNombre(DocumentoZona.getString(
                            "nombre"));
                    zona.setExtension(DocumentoZona.getDouble(
                            "extension").floatValue());
                    zonas.add(zona);
                }
                itinerario.setZonas(zonas);
                List<Document> guiasDoc = (List<Document>) documento.get(
                        "guias");
    ...
    public List<Itinerario> consultarItinerariosUltimoMes() {
        ...
        for (Document documento : iterable) {
            ...
            itinerario.setId(documento.getObjectId("_id"));
            itinerario.setNombre(documento.getString("nombre"));
            itinerario.setDuracionRecorrido(documento.getInteger(
                    "duracionRecorrido"));
            itinerario.setLongitud(documento.getDouble(
                    "longitud").floatValue());
            itinerario.setMaxNumVisitantes(documento.getInteger(
                    "maxNumVisitantes"));
            itinerario.setFechaHoraItinerario(documento.getDate(
                    "fechaHoraItinerario"));
            itinerario.setZonas(documento.getList("zonas", 
                    Zona.class));
            itinerario.setGuias(documento.getList("guias", 
                    Guia.class));
        ...
        for (Itinerario itinerario : todosItinerarios) {
            if (itinerario.getFechaHoraItinerario().after(fechaUltimoMes)) {
                if (itinerario.getFechaHoraItinerario().getYear() == 
